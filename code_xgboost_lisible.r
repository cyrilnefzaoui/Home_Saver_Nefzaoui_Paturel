...
test_purified=(vrai_test %>% select(-c(Id,rv1,rv2,date,DayType,Instant,lights,Visibility,InstantF,Posan,Month,Heure,BE_load_actual_entsoe_transparency,
...
total_purified=(total_sorted %>% select(-c(Id,is_in_test,rv1,rv2,date,DayType,Instant,lights,Visibility,InstantF,Posan,Month,Heure,BE_load_actual_entsoe_transparency,
                                           BE_load_forecast_entsoe_transparency,BE_wind_onshore_capacity,BE_wind_onshore_capacity,BE_wind_onshore_generation_actual,BE_wind_onshore_profile,WeekStatus)))
...
xgbGrid <- expand.grid(nrounds = 1000,  max_depth = 20,colsample_bytree = seq(0.5, 0.6, length.out = 5), eta = 0.1,gamma=0,min_child_weight = 1,subsample = 0.5)
set.seed(0)
...
X_train_xgb=xgb.DMatrix(as.matrix(train), label=Appliances_reference)




xgb_model = xgb.train(params=list(booster="gbtree"),data=X_train_xgb , trControl = xgb_trcontrol, tuneGrid = xgbGrid_heavy,  method = "xgbTree",nrounds=1000,verbose=2,print_every_n = 1)
...
new_test=missForest(test_purified, maxiter = 1000, ntree = 1000, variablewise = FALSE,
                    decreasing = FALSE, verbose = TRUE,
...
xts_predictions=xts(ts_predictions,order.by=date_vrai_test)
