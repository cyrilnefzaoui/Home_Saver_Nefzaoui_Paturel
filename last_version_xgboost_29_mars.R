...
prediction_MissForest_10=xts(read.csv(file="submission_mf_bertrand_cyril.csv", sep=",", dec='.')$Appliances[4655:5771],order.by=date_test[4655:5771])


prediction_xgboost_simple_struct[10]



p_xgb=xts((read.csv(file="submission_simple_xgboost.csv", sep=",", dec='.')$Appliances),order.by=date_test)
prediction_regression_lineaire1=(read.csv(file="reg72.7.csv", sep=",", dec='.')$Appliances[1:4654])
prediction_regression_lineaire2=(read.csv(file="submission_72.9.csv", sep=",", dec='.')$Appliances[4655:5771])
...
integrate(read.csv(file="reg_726.csv", sep=",", dec='.')$Appliances[4655:5771])
p_moy=(p2+p3+p4)/3
...
plot(ts(p_moy$V1[which(p_moy$V1<80)]))
plot(ts(p_moy_original))
lines(ts(p_moy$V1),
      col="red")

plot()
p_moy_to_use=p_moy[which(80) ]plot()
...
plot(appliances_imputed)
p_mf_permuted=xts((read.csv(file="submission_mf500_permuted.csv", sep=",", dec='.')$Appliances),order.by=date_test)
appliances_bertrand=read.csv(file="submission_72.9.csv", sep=",", dec='.')$Appliances[1:5771]
app=read.csv(file="submission_composite_xgboost.csv", sep=",", dec='.')$Appliances[4655:5771]
...
rmse(appliances_imputed,prediction_regression_lineaire1)
...
help("dygraph")
graphes=cbind(prediction_regression_lineaire,prediction_MissForest_10,app)
dygraph(graphes,main="Comparaison de deux méthodes de prévision:Regression linéaire et Méthode permutation-imputation avec MissForest: 10 arbres")
dygraph(graphes)
new_predictions
...
##############completion par foret aléatoires
  which(is.na(total_heavy_numeric_df$Appliances))
...
which(is.na(total_heavy_numeric_df_permuted))

co
total_heavy_numeric_df_purified=(total_heavy_numeric_df %>% select(-c(lights,Tdewpoint,RH_6,Visibility,Instant,DayType,BE_load_actual_entsoe_transparency,
...
permut=total_heavy_numeric_df_purified[permutation]

length(which(is.na(permut$Appliances)))

length(which(is.na(total_heavy_numeric_df_purified$Appliances)))



plot(total_heavy_numeric_df_purfied$Appliances)
length(which(is.na(total_heavy_numeric_df_purfied$Appliances)))
total_imputed_permut=missForest(permut, maxiter = 100 ,ntree = 200, variablewise = TRUE,
                     decreasing = FALSE, verbose = TRUE,
                     
                     mtry =sqrt(ncol(permut)),
                     replace = TRUE,
...
xts_final=xts()
new_app=data_complete10$Appliances[18619:19735]
plot(ts(data_complete10$Appliances[18618:19635]))


xts_pred=xts(ts(data_complete10$Appliances[18619:19735]),order.by=date_test[4655:5771])
dygraph(xts_pred,main="Previsions pures obtenues par complétion,sans permutation")
data_just_imputed10=data_complete30[which(data_complete10$is_in_test==2)]
tokaggle=c(appliances_imputed,new_app)
length(tokaggle)
...
total_heavy_numeric_df_permuted_purified=(total_heavy_numeric_df_permuted %>% select(-c(lights,Tdewpoint,RH_6,Visibility,Instant,DayType,BE_load_actual_entsoe_transparency,
                                                                                        BE_load_forecast_entsoe_transparency,BE_wind_onshore_capacity,BE_wind_onshore_capacity,BE_wind_onshore_generation_actual,BE_wind_onshore_profile,Month,Windspeed,Press_mm_hg,InstantF,T2,T3,RH_2,RH_8,Day_of_week,Posan)))
to_train=total_heavy_numeric_df_permuted_purified[which(is_in_test<2)]
...
y_train=to_train$Appliances
total_heavy_numeric_df_permuted_purified_wthApp=(to_train %>% select(-Appliances))
X_train_permuted=(xgb.DMatrix(as.matrix(total_heavy_numeric_df_permuted_purified_wthApp ),label=y_train))
...
X_test_purified=total_heavy_numeric_df_permuted_purified[which(is_in_test==2)]
X_test_purified=arrange(X_test_purified, as.Date(X_test_purified$date))
X_test_purified=(X_test_purified  %>% select (-Appliances))
X_test_purified=xgb.DMatrix(as.matrix(X_test_purified))
prediction_futur=predict(xgb_model,newdata=X_test_purified)
moy=moy+prediction_futur
}
return(moy/n)
"
}
...
xgb_model=xgb.train(params=list(booster="gbtree"),data=X_train_permuted, trControl = xgb_trcontrol, tuneGrid = xgbGrid,  method = "xgbTree",nrounds=10000,verbose=2,print_every_n = 1)
#####################validation
...
X_test_purified$Appliances=prediction_futur
...
d
rmse(prediction_futur,appliances_bertrand[4655:5771])
plot(appliances_bertrand[4655:5771])
lines(prediction_futur)
plot(prediction_futur)
...
write.table(submit, file="submission_completion_partielle.csv", quote=F, sep=",", dec='.',row.names = F)
plot(tokaggle)
